...
curl -X POST http://localhost:8000/api/v1/auth/init \
  -H "Content-Type: application/json" \
  -d '{"email": "admin2@test.com", "name": "Admin2"}'
```
...
```bash
curl -X POST http://localhost:8000/api/v1/mcps/analyze \
  -H "Content-Type: application/json" \
  -H "X-API-Key: <admin_key>" \
  -d '{"git_url": "<url to a repo containing FastMCP server>"}'
```
...
curl -X POST http://localhost:8000/api/v1/mcps/analyze \
  -H "Content-Type: application/json" \
  -H "X-API-Key: <admin_key>" \
  -d '{"git_url": "https://github.com/some/non-mcp-repo.git"}'
```
...
curl -X POST http://localhost:8000/api/v1/mcps/analyze \
  -H "Content-Type: application/json" \
  -H "X-API-Key: <admin_key>" \
  -d '{"git_url": "https://github.com/nonexistent/repo.git"}'
```
Expected: 200, returns empty metadata (graceful failure)

### T2.4 — Submit MCP (Valid)
```bash
curl -X POST http://localhost:8000/api/v1/mcps/submit \
  -H "Content-Type: application/json" \
  -H "X-API-Key: <admin_key>" \
  -d '{
...
### T2.8 — Review Show
```bash
curl http://localhost:8000/api/v1/review/<listing_id> \
  -H "X-API-Key: <admin_key>"
```
...
```bash
curl -X POST http://localhost:8000/api/v1/review/<listing_id>/approve \
  -H "X-API-Key: <admin_key>"
```
...
```bash
curl -X POST http://localhost:8000/api/v1/review/<new_listing_id>/reject \
  -H "Content-Type: application/json" \
  -H "X-API-Key: <admin_key>" \
  -d '{"reason": "Description is misleading"}'
```
...
```bash
curl http://localhost:8000/api/v1/review \
  -H "X-API-Key: <non_admin_key>"
```
Expected: 403 `"Admin access required"`

### T2.20 — Install Unapproved Listing
```bash
curl -X POST http://localhost:8000/api/v1/mcps/<pending_listing_id>/install \
  -H "Content-Type: application/json" \
  -H "X-API-Key: <admin_key>" \
  -d '{"ide": "cursor"}'
```
